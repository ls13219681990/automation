...
public class MainController {
    ...
    public String main(Model model) {
        ...
        List<Menu> userMeun = new ArrayList<>();
        if (!user.getId().equals(AdminConst.ADMIN_ID)) {
            user.getRoles().forEach(role -> {
                role.getMenus().forEach(menu -> {
                    if (!menu.getEngineering().equals(1)) {
                        userMeun.add(menu);
                    }
                });
            });
    ...
    public List<Object> tree() {
        ...
        if (!user.getId().equals(AdminConst.ADMIN_ID)) {
            user.getRoles().forEach(role -> {
                role.getMenus().forEach(menu -> {
                    if (!menu.getEngineering().equals(1)  && !menu.getType().equals((byte)4)) {
                        userMeun.add(menu);
                    }
                });
            });
        ...
        while (iterator.hasNext()){
            ...
            if(menu.getUrl().indexOf("/system/measuringSpot/index?nodeName=") != -1){
                iterator.remove();
            }
       }
        ...
        List<Object> objects = MenuTreeUtil.menuList(CommonMethod.removeDuplicateWithOrder(menuList));
        System.out.println();
        return objects;
    }
