...
public class RoleController {
    ...
    @Autowired
    RoleWorkSpotService roleWorkSpotService;

    @Autowired
    WorkSpotService workSpotService;
    ...
    public ResultVo auth(
        ...
        //删除该角色下面的所有工点然后再添加
        roleWorkSpotService.removeByRoleId(role.getId());

            //获取所有工点
            for (Menu menu : menus) {
                if (menu.getUrl().contains("workSpot")) {
                    WorkSpot workSpot = workSpotService.findByName(menu.getTitle());
                    if (workSpot != null ) {
                        RoleWorkSpot roleWorkSpot = new RoleWorkSpot();
                        roleWorkSpot.setRoleId(role.getId());
                        roleWorkSpot.setWorkSpotId(workSpot.getId());
                        roleWorkSpotService.save(roleWorkSpot);
                    }
                }
            }
        ...
        for (Menu m : role.getMenus()) {
            Menu menu = menuService.findByTitleAndEngineering(m.getTitle(), 1);
            if (menu != null) {
                menuSet.add(menu);
            }
        }

        role.getMenus().addAll(menuSet);
        // 保存数据
        roleService.save(role);
