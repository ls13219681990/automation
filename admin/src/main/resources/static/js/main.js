layui.use(['element', 'form', 'layer', 'upload'], function () {
     ...
     * 刷新父级的父级页面
     */
    form.on('submit(ajaxSubmitParentParent)', function(e){
     ...
     * 刷新父级的父级页面
     */
    form.on('submit(SubmitParentParent)', function(e){
        var tree = e.field.tree;
        var form = $(this).parents("form");
        var url = form.attr("action");
        var serializeArray = form.serializeArray();
        $.post(url, serializeArray, function (result) {
    ...
    form.on('submit(ajaxSubmit)', function(e){
    ...
    /* 提交表单数据并刷新父菜单 */
    $(document).on("click", ".ajax-submit-refresh", function (e) {
        e.preventDefault();
        var form = $(this).parents("form");
        var url = form.attr("action");
        var serializeArray = form.serializeArray();
        $.post(url, serializeArray, function (result) {
            ...
            parent.layui.table.reload('acquisitionTable',{page: {curr: 1}});
        });
    });
    ...
    $(document).on("click", ".open-node-popup, .open-popup-node-param", function () {
        var title = $(this).data("title");
        var url = $(this).attr("data-url");
        if ($(this).hasClass("open-popup-node-param")) {
            var tdcheckbox = $(".timo-table td .timo-checkbox :checkbox:checked");
            var param = '';
            if (tdcheckbox.length === 0) {
                layer.msg('请选择一条记录');
